...
$searchedQuery = htmlspecialchars(app('request')->input('queue'));

?>

@extends('layouts.app')

@section('title', "Searched for '$searchedQuery'")
			            ...
			            <div class="product-grid3">
			                <div class="product-image3">
			                    ...
			                    <a href="{{ route('store.show', ['id' => $product->id, 'slug' => $product->slug]) }}">
									<img class="pic-1 img-product-2" src="{{ asset('storage/'. $img1) }}" class="pr-thumbnail img-fluid" alt="{{ $product->name }}">
									@if ($img2 != null)
										<img class="pic-2 img-product-2" src="{{ asset('storage/'. $img2) }}" class="pr-thumbnail img-fluid" alt="{{ $product->name }}">
									@endif
			                    </a>
			                    <ul class="social">
